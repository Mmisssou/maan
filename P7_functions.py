...
def data_preprocessing(df, var_model, var_target, enc_strat_cat='label'):
    ...
    if df[var_model].isna().sum().sum() != 0:
        ...
        if len(list(num_cols)):
            df_train[num_cols] =                       StandardScaler().fit_transform(df_train[num_cols].values)
...
def plot_projection(X, y, model=None, plot_only_idx=None,
    ...
    # a2 - if no model but ser_clust is given, plot with colors
        ...
        dict_proj, dict_proj_centers, _ =             prepare_2D_axes(X, y, ser_clust=ser_clust, proj=[proj],
...
def plot_hyperparam_tuning_optuna(gs, grid_params, params=None, score='score', figsize=(12, 7),
    ...
    li_param = [param_name for param_name in grid_params.keys()                 if df['params_'+param_name].dtype in li_type]
...
def plot_2D_hyperparam_tuning_optuna(scv, params=None, score=None, annot=False, cmap=None,
    ...
    if params:  # example: params=['enet__alpha', 'enet__l1_ratio']
        ...
        if len(params_scv)!=2:
            print('WARNING : parameters to display were guessed,                provide the params parameter with 2 parameters')
...
def plot_learning_curve(name_reg, estimator, X, y, ylim=None, cv=None,
    ...
    if dict_learn_curves.get(name_reg, np.nan) is not np.nan:
        ...
        train_sizes, train_scores, test_scores, fit_times =             list(zip(*list(dict_learn_curves[name_reg].items())))[1]
...
class CustTransformer(BaseEstimator):
    ...
    def d_type_col(self, X):
        ...
        C_l_card_cols =             X_C_cols.nunique()[X_C_cols.nunique()
    ...
    def fit(self, X, y=None):
        ...
        if self.has_num and self.has_cat:
            ...
            self.column_trans =                 ColumnTransformer([('num', self.num_trans, self.num_cols)])
        elif not self.has_num and self.has_cat:
            self.column_trans = ColumnTransformer([('cat', self.cat_trans, self.cat_cols)])
        else:
